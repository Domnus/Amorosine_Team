class Tabelas {
	...
	criarVoluntariosEvento() {
		const sql = 'CREATE TABLE IF NOT EXISTS `amorosine_team`.`Voluntarios_Evento` (`idEvento` INT NOT NULL, `idVoluntario` INT NOT NULL, `idEndereco` INT NOT NULL, PRIMARY KEY (`idEvento`, `idVoluntario`, `idEndereco`), INDEX `fk_Voluntarios_Evento_Voluntarios1_idx` (`idVoluntario` ASC, `idEndereco` ASC) VISIBLE, CONSTRAINT `fk_Voluntarios_Evento_Eventos1` FOREIGN KEY (`idEvento`) REFERENCES `amorosine_team`.`Eventos` (`idEvento`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_Voluntarios_Evento_Voluntarios1` FOREIGN KEY (`idVoluntario` , `idEndereco`) REFERENCES `amorosine_team`.`Voluntarios` (`idVoluntario` , `idEndereco`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE = InnoDB;'
