class Tabelas {
	...
	criarCidades() {
		const sql = 'CREATE TABLE IF NOT EXISTS `amorosine_team`.`Cidades` (`idCidade` INT NOT NULL AUTO_INCREMENT, `descricao` VARCHAR(45) NOT NULL, `UF` VARCHAR(2) NOT NULL, PRIMARY KEY (`idCidade`)) ENGINE = InnoDB;'
	...
	criarEnderecos() {
		const sql = 'CREATE TABLE IF NOT EXISTS `amorosine_team`.`Enderecos` (`idEndereco` INT NOT NULL AUTO_INCREMENT, `idCidade` INT NOT NULL, `rua` VARCHAR(45) NOT NULL, `numero` VARCHAR(45) NOT NULL,`bairro` VARCHAR(45) NOT NULL, `complemento` VARCHAR(45) NULL, `CEP` VARCHAR(8) NOT NULL, PRIMARY KEY (`idEndereco`, `idCidade`), INDEX `fk_Enderecos_Cidades1_idx` (`idCidade` ASC) VISIBLE, CONSTRAINT `fk_Enderecos_Cidades1` FOREIGN KEY (`idCidade`) REFERENCES `amorosine_team`.`Cidades` (`idCidades`)ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE = InnoDB;'
